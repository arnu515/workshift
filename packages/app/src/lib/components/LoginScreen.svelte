<script lang="ts">
  ...
  async function getLoginType(email: string): Promise<typeof loginType> {
    const res = await axios.post("/auth/isregistered", { email });
    ...
    if (res.data.isRegistered && res.data.provider !== "local") {
    ...
    loginType = res.data.isRegistered ? "login" : "register";
  }

  async function auth() {
    loading = true;
    if (loginType === "none") {
      await getLoginType(email);
      loading = false;
      return;
    }

    const res = await axios.post(`/auth/local/${loginType}`, {
      email,
      password,
      username
    });
    ...
    if (!res.status.toString().startsWith("2")) {
      toast.push(
        Array.isArray(res.data.message) ? res.data.message.join("\n") : res.data.message
      );
      return;
    }
    $user = res.data;
  ...
  {:else if loginType === "login"}
    <form on:submit|preventDefault={auth}>
      ...
      <div class="my-2 flex items-center gap-2">
        ...
        <button type="submit" disabled={loading} class:loading>Login</button>
      </div>
    </form>
  {:else if loginType === "register"}
    <form on:submit|preventDefault={auth}>
      ...
      <div class="my-2 flex gap-2 items-center">
        ...
        <button type="submit" disabled={loading} class:loading>Register</button>
      </div>
    </form>
  {/if}
  <p class="text-center text-gray-500 uppercase mt-5 mb-2">
    Or use a third-party service
  </p>

  <div class="flex flex-col gap-2 justify-center">
    <a
      href="{axios.defaults.baseURL}/connect/github"
      class="provider bg-[#181717] text-white"
    >
      <svg
        role="img"
        viewBox="0 0 24 24"
    ...
    <a
      href="{axios.defaults.baseURL}/connect/discord"
      class="provider bg-[#5865f2] text-white"
    >
      <svg
        role="img"
        viewBox="0 0 24 24"
